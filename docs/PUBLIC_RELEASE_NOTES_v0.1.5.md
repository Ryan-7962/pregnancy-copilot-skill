# Public Release Notes v0.1.5

Pregnancy Copilot Skill v0.1.5 is the first handoff package suitable for external technical testing with Hermes/OpenClaw-style host Agents, with an added daily metrics memory index for high-frequency pregnancy records.
...
- Added `memory/daily_metrics.yaml` and `memory/daily_metrics.md` for high-frequency daily records.
- Indexed weight points with `latest`, `previous`, and `delta_kg` so weight changes are available without scanning raw chat history.
- Indexed recent mood, diet, activity, and sleep summaries from official non-private events.
- Added daily metrics to regenerated `current_context.md` under "高频日常指标摘要".
- Kept daily metrics separate from structured medical observations. Weight/mood logs are context and trend data, not automatically doctor-confirmed medical facts.
- Hardened medical memory: same metric plus same measured date now uses later `recorded_at` as current; missing `measured_at` is rejected instead of guessed.
- Strengthened host instructions to say "unknown / needs more information" when facts are absent, stale, or not source-backed.

## What Changed Earlier In v0.1.4
...
v0.1.6 or v0.2 should focus on real-host integration hardening:
